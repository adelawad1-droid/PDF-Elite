...
import { ToolType, PDFTool } from './types';
...
const SEOKeywords = () => (
  <section className="bg-white py-10 border-t border-gray-100 mt-12">
    <div className="max-w-7xl mx-auto px-4 text-center">
      <div className="flex flex-wrap justify-center gap-4 text-xs md:text-sm text-gray-400 font-medium opacity-60">
        <span>PDF to Word</span>
        <span>Merge PDF</span>
        <span>Compress PDF</span>
        <span>Split PDF</span>
        <span>Edit PDF Online</span>
        <span>تحويل PDF إلى Word</span>
        <span>دمج ملفات PDF</span>
        <span>ضغط حجم PDF</span>
      </div>
    </div>
  </section>
);
  ...
  <section className="bg-white py-20 border-b border-gray-100 overflow-hidden">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center">
      <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-50 text-red-600 text-sm font-semibold mb-6 animate-pulse">
        <Cpu className="w-4 h-4" /> {t.hero_title.includes('PDF') ? t.hero_title : t.hero_title}
      </div>
        ...
        {!selectedTool ? (
          ...
          <div className="max-w-4xl mx-auto px-4 py-12">
            <button 
              ...
              className={`flex items-center gap-2 text-gray-500 hover:text-red-600 mb-8 transition-colors`}
            >
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
              <div className="bg-gray-50 p-8 border-b border-gray-100 flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">{tools.find(t => t.id === selectedTool)?.title}</h2>
                  ...
                  <div className="space-y-6">
                    ...
                    {selectedTool === 'ai-chat' && (
                      <div className="space-y-4">
                        <div className="h-64 overflow-y-auto bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-4">
                          {chatHistory.length === 0 && (
                            <p className="text-gray-400 text-center mt-20">{t.chat_start}</p>
                          )}
                          {chatHistory.map((msg, idx) => (
                            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                              <div className={`max-w-[80%] px-4 py-2 rounded-2xl ${msg.role === 'user' ? 'bg-red-600 text-white rounded-tl-none' : 'bg-white border border-gray-200 text-gray-900 rounded-tr-none'}`}>
                                {msg.text}
                              </div>
                            </div>
                          ))}
                          {processing && <div className="text-gray-400 animate-pulse text-sm">{t.ai_thinking}</div>}
                        </div>
                        <div className="flex gap-2">
                          <input 
                            ...
                            className="flex-grow bg-white border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-red-500"
                          />
                          <button 
                            onClick={handleChat}
                            className="bg-red-600 text-white p-3 rounded-xl hover:bg-red-700 transition-colors shadow-lg shadow-red-100"
                          >
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 text-gray-400 text-sm">
